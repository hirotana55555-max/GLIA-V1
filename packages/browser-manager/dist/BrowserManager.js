...
class BrowserManager {
    ...
    constructor(policyOverride) {
        ...
        this.policy = { ...types_1.DEFAULT_RECYCLING_POLICY, ...(policyOverride || {}) };
        ...
        for (const bId of browserIds) {
            ...
            try {
                ...
                if (opts.forceKillProcesses && bRec.pid) {
                    ProcessManager_1.ProcessManager.killPID(bRec.pid, true);
                }
            }
    ...
    async createContext() {
        ...
        const browserId = `b-${(0, uuid_1.v4)()}`;
        const pid = ProcessManager_1.ProcessManager.getBrowserPID(browser);
        const browserRecord = {
            id: browserId,
            browser,
            pid,
     ...
     */
    findContextIdForContext(context) {
