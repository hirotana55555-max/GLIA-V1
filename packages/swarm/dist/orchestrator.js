...
const audit_1 = require("@glia/audit");
const sie_1 = require("@glia/sie");
const api_client_1 = require("@glia/api-client");
const prompt_core_1 = require("@glia/prompt-core");
class SwarmOrchestrator {
    constructor(config) {
        this.audit = new audit_1.AuditLogger("./logs", "swarm-audit.jsonl");
        this.pool = new browser_manager_1.ResourcePool({ maxSize: 2, model: "chromium" });
        this.sieExecutor = new sie_1.SIEExecutorV2(this.pool, this.audit);
        ...
        this.apiClient = new api_client_1.GliaApiClient(this.config.apiKey);
        this.parser = new sie_1.SIEParser();
        this.validator = new sie_1.SIEValidator();
    }
    async runMission(mission) {
        try {
            ...
            if (proposal.instructions && proposal.instructions.length > 0) {
                ...
                const approvalCallback = async (risky) => {
    ...
    async manageHistoryContext(history) {
        if (history.length === 0)
            return "";
        ...
        context += `[Recent Failed Attempts]:\n` + recentHistory.map(h => `Attempt #${h.attemptIdx}:
             Score: ${h.critique.score}
             Critique: ${h.critique.feedback}
             Proposal Plan: ${h.proposal.content}`).join("\n\n");
        return context;
    }
    async generateProposal(mission, historyContext) {
        ...
        const systemPrompt = (0, prompt_core_1.synthesizePrompt)({
...
Return a JSON Proposal adhering strictly to the provided schema.`,
        ...
        try {
            ...
            const result = await this.apiClient.jsonCompletion(messages, { model: "openai/gpt-3.5-turbo" });
    ...
    async runCritique(mission, proposal, history) {
        const schemas = [{
                id: "critique-schema",
...
Score 0-100. < 70 is fail.`,
        ...
        const result = await this.apiClient.jsonCompletion(messages, { model: "openai/gpt-4" } // Critique should use stronger model?
        );
    ...
    getMockCritique(mission, proposal) {
        return {
