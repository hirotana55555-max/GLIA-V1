...
find "$ROOT" -maxdepth 3 -type f \( \
    ...
    -o -name "*.config.ts" \
    -o -name "*.config.json" \
\) ! -path "*/node_modules/*" ! -path "*/.git/*" ! -path "*/backups/*" ! -path "*/dist/*" ! -path "*/SPECIFICATION/*" 2>/dev/null | sed "s|$ROOT/||" | sort
echo ""
# Source Directories
for dir in packages apps; do
    if [ -d "$ROOT/$dir" ]; then
        ...
        find "$ROOT/$dir" -maxdepth 2 -type f \( -name "*.ts" -o -name "*.js" \) \
            ! -path "*/node_modules/*" ! -path "*/.git/*" ! -path "*/backups/*" ! -path "*/dist/*" ! -path "*/SPECIFICATION/*" 2>/dev/null \
            | sed "s|$ROOT/||" | head -15
        echo
    fi
done
...
find "$SPEC_DIR" -maxdepth 2 -type f \( -name "*.md" -o -name "*.txt" -o -name "*.sh" \) \
    ! -path "*/node_modules/*" ! -path "*/.git/*" ! -path "*/backups/*" 2>/dev/null | sed "s|$ROOT/||" | sort
echo '```'
echo ""
echo "## 📌 4. Git 状態（簡潔版）"
echo '```'
