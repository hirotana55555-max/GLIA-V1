...
- **手順**:
    ...
    4.  **実行**: `node scripts/integration-test/antigravity-harness.js` を実行。
    5.  **確認**: 画面に「SUCCESS: Mission accepted」と表示されれば成功。ログファイル（`logs/swarm-audit.jsonl`）でAIの思考と行動履歴を確認。
...
```bash
# 統合テストハーネスの実行
node scripts/integration-test/antigravity-harness.js
```

**実行されること:**
1.  **Mission発令**: 「GoogleでDeepMindを検索せよ」等の指令が出ます。
2.  **思考（Orchestrator）**: AI（または模擬AI）が「検索ページに行って...」という手順書を作成します。
3.  **批評（Critique）**: 別のAI（Reviewer）が手順書を審査し、スコアを付けます（Phase 2新機能）。
4.  **審査（SIE）**: 危険な操作がないか、システムが自動チェックします。
5.  **実行（BrowserManager）**: 実際にブラウザ操作（のシミュレーション）が行われます。
6.  **結果判定**: 成功したかどうかが判定されます。
